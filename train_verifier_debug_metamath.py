...
def main():
    ...
    checkpoint_dir = f"/root/OVM/models/metamath/generators/{generator_id}"
    save_verifier_id=f"n{n_solution}-scahead-mse-lm-token"

    experimentID = 1
    save_dir=f"/root/OVM/models/metamath/verifiers/{generator_id}-{experimentID}"
    final_id=f"{generator_id}-{save_verifier_id}"

    os.environ['WANDB_PROJECT'] = 'metamath-Verifier'
    os.environ['WANDB_NAME'] = f"{generator_id}-{experimentID}"

    args = [
        "--model_name_or_path", "facebook/opt-125m",
        "--data_dir", "/root/OVM/data/metamath/model_generation/",
